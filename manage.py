...
def seeder():
        ...
        for row in csvreader:
            try:
                CompanyProducts.create(
                    int(row[0]), row[1], int(row[2]), float(row[3]), row[4], int(row[5])
                )
            except:
                pass
        ...
        for row in csvreader:
            try:
                CompanyOrders.create(
                    int(row[0]),
                    datetime.strptime(row[1], "%b %d %Y %I:%M:%S:%f%p"),
                    int(row[2]),
                    float(row[3]),
                    int(row[4]),
                )
    ...
    with open("app/editor_data/order_items.csv", "r") as f:
        ...
        for row in csvreader:
            try:
                OrderItems.create(
                    int(row[0]), int(row[1]), int(row[1]), float(row[1]), int(row[1])
                )
            except:
                pass

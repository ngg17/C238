...
def login():
    try:
        ...
        if user:
            ...
            return jsonify({"status": "success", "id": user[0]}), 200
        else:
            return jsonify({"status": "error", "message": "Not sure"}), 400
...
def execute():
    try:
        ...
        if len(result) == 0:
            return jsonify({"status": "no_result"}), 200
        else:
            ...
            return jsonify({"status": "success", "keys": keys, "values": values}), 200
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 400
